...
begin {
	...
	$hshManifestParams = @{
		...
		Description = "Module with the super useful functions that were previously scattered about the web by various PowerShellers in the world"
		FileList = Write-Output "${strModuleName}.psd1" "${strModuleName}_functions.psm1" "en-US\about_${strModuleName}.help.txt"
		FunctionsToExport = Write-Output ConvertFrom-UnattendEncodedPassword ConvertTo-UnattendEncodedPassword Get-ArgumentCompleter Get-DataFromMemoryStream Get-EffectiveFSAccessRule Get-FileEndOfLineType Get-ParameterSetInformation Get-ResourceUtilization Get-StringCasePermutation_Recursive Get-StringCasePermutation Get-Weather Invoke-ActivatePythonVirtualenv New-CertificateSigningRequest New-MarkdownCommandExample Optimize-PSReadlineHistory Start-Demo Test-ArgumentCompleter
		IconUri = "http://static.vnugglets.com/imgs/vNuggletsLogo.jpg"
		LicenseUri = "https://github.com/mtboren/PS.Utils/blob/main/License"
		...
		ProjectUri = "https://github.com/mtboren/PS.Utils"
		ReleaseNotes = "See release notes at https://github.com/mtboren/PS.Utils/blob/main/ChangeLog.md"
		Tags = Write-Output Utils Utilities FaF Handy Convenient Cornvenient 🌽
		# Verbose = $true
	} ## end hashtable
} ## end begin
